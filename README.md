![FastFile](https://cdn.discordapp.com/attachments/758257695458983947/777473345905491979/FastFile-web.png)

# Back-end

## About

## API

---

### Table of contents

-   [Users](/docs/Users.md):
    -   [User:](/docs/Users.md/#user)
        -   [Login](/docs/Users.md/#login)
        -   [Register](/docs/Users.md/#register)
        -   [Logout](/docs/Users.md/#logout)
        -   [Account information](/docs/Users.md/#account-information)
        -   [Change password](/docs/Users.md/#change-password)
        -   [Delete account](/docs/Users.md/#delete-account)
    -   [Admin](/docs/Users.md/#admin)
        -   [All users](/docs/Users.md/#all-users)
        -   [One user](/docs/Users.md/#one-user)
        -   [User role](/docs/Users.md/#user-role)
        -   [Change password](/docs/Users.md/#change-password---admin)
        -   [Delete user](/docs/Users.md/#delete-user)
-   [Files](/docs/Files.md)
    -   [User:](/docs/Files.md/#user)
        -   [All files](/docs/Files.md/#all-files)
        -   [One file](/docs/Files.md/#one-file)
        -   [Upload file](/docs/Files.md/#upload-file)
        -   [Download file](/docs/Files.md/#download-file)
        -   [Rename file](/docs/Files.md/#rename-file)
        -   [Remove file](/docs/Files.md/#remove-file)
        -   [View image](/docs/Files.md/#view-image)
    -   [Admin:](/docs/Files.md/#admin)
        -   [All files](/docs/Files.md/#all-files---admin)
        -   [One file](/docs/Files.md/#one-file---admin)
        -   [Upload file](/docs/Files.md/#upload-file---admin)
        -   [Download file](/docs/Files.md/#download-file---admin)
        -   [Rename file](/docs/Files.md/#rename-file---admin)
        -   [Remove file](/docs/Files.md/#remove-file)
        -   [View image](/docs/Files.md/#view-image---admin)
-   [Folders](/docs/Folders.md)
    -   [User:](/docs/Folders.md/#user)
        -   [All items in folder](/docs/Folders.md/#all-items)
        -   [Create folder](/docs/Folders.md/#create-folder)
        -   [Remove folder](/docs/Folders.md/#remove-folder)

---
